import { CiLocationArrow1 } from "react-icons/ci";

const NewsLetter = () => {
  return (
    <div
      style={{ backgroundImage: "url(/hero-4.webp)" }}
      className="flex relative bg-center bg-cover flex-col min-h-screen md:p-25 p-5 gap-3 items-center justify-center "
    >
      <div className="absolute inset-0 bg-black opacity-40"></div>
      <div className="flex flex-col justify-center items-center gap-3 z-10">
        <p className="text-sm z-5 text-[#D47784] font-semibold">
          STAY CONNECTED
        </p>
        <p className="font-[Bastoni] text-center text-3xl text-white">
          Join Our Inner Circle
        </p>
        <p className="text-[white] text-center">
          Be the first to know about new acquisitions, private sales, and
          exclusive events.
        </p>
        <div className="flex md:flex-row flex-col gap-2">
          <a
            href="https://chat.whatsapp.com/EurJVZmIYrz2HRXf0lI8mW"
            target="_blank"
            rel="noopener noreferrer"
            className="border border-[#D47784] px-6 py-2 flex gap-2 justify-center items-center  font-medium bg-[#D47784] text-white transition duration-300 "
          >
            <CiLocationArrow1 size={20} /> Join
          </a>
        </div>
        <p className="text-[white] text-[12px]">
          By subscribing, you agree to our Privacy Policy and consent to receive
          updates.
        </p>
      </div>
    </div>
  );
};

export default NewsLetter;
